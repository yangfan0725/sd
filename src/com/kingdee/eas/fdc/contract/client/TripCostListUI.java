 ...
 * output class name
 */
public class TripCostListUI extends AbstractTripCostListUI
{
    private static final Logger logger = CoreUIObject.getLogger(TripCostListUI.class);
    
    /**
     * output class constructor
     */
    public TripCostListUI() throws Exception
    {
        super();
    }
    ...
    public void onLoad() throws Exception {
			...
			public void afterDataFill(KDTDataRequestEvent e) {
				...
				try {
					hasFunctionPermission = PermissionFactory.getRemoteInstance().hasFunctionPermission(
							...
							new MetaDataPK(TripApplyListUI.class.getName()),
		...
		this.tblTripCost.getSelectManager().setSelectMode(KDTSelectManager.ROW_SELECT);
		this.tblTripCost.setEnabled(false);
		
    	this.actionTraceUp.setVisible(false);
    	this.actionCreateTo.setVisible(false);
    	
    	this.tblMain.getColumn("day").getStyleAttributes().setHorizontalAlign(HorizontalAlignment.RIGHT);
    	this.tblTripCost.getColumn("day").getStyleAttributes().setHorizontalAlign(HorizontalAlignment.RIGHT);
	...
	public void actionEdit_actionPerformed(ActionEvent e) throws Exception {
		int[] selectRows = KDTableUtil.getSelectedRows(this.tblTripCost);
		if(selectRows==null || selectRows.length==0){
			FDCMsgBox.showWarning(this, EASResource.getString(FrameWorkClientUtils.strResource + "Msg_MustSelected"));
			return;
		}
		int rowIndex = this.tblTripCost.getSelectManager().getActiveRowIndex();
		IRow row = this.tblTripCost.getRow(rowIndex);
		TripCostInfo info=(TripCostInfo) row.getUserObject();
		...
		IUIWindow uiWindow = UIFactory.createUIFactory(UIFactoryName.NEWTAB).create(TripCostEditUI.class.getName(), uiContext, null, OprtState.EDIT);
		uiWindow.show();
	}
	public void actionRemove_actionPerformed(ActionEvent e) throws Exception {
		...
		if(confirmRemove()){
			TripCostFactory.getRemoteInstance().delete(new ObjectUuidPK(info.getId()));
	...
	public void actionAudit_actionPerformed(ActionEvent e) throws Exception {
		...
		TripCostFactory.getRemoteInstance().audit(info.getId());
		FDCClientUtils.showOprtOK(this);
		queryList();
	}
	public void actionUnAudit_actionPerformed(ActionEvent e) throws Exception {
		...
		TripCostFactory.getRemoteInstance().unAudit(info.getId());
		FDCClientUtils.showOprtOK(this);
		queryList();
	}
...
//		mainQuery.setFilter(filter);
	...
	protected String getEditUIName() {
		return TripCostEditUI.class.getName();
	}
	...
	public void actionAddNew_actionPerformed(ActionEvent e) throws Exception {
		...
		if(rowIndex>=0){
			String id  = this.tblMain.getRow(rowIndex).getCell("id").getValue().toString();
			if(TripCostFactory.getRemoteInstance().exists("select * from where tripApply.id='"+id+"'")){
	...
	@Override
	protected void tblTripCost_tableClicked(KDTMouseEvent e) throws Exception {
		if (e.getType() == KDTStyleConstants.BODY_ROW && e.getButton() == MouseEvent.BUTTON1 && e.getClickCount() == 2){
			...
			uiContext.put(UIContext.OWNER, this);
			uiContext.put("ID", info.getId().toString());
			IUIWindow uiWindow = UIFactory.createUIFactory(UIFactoryName.NEWTAB).create(TripCostEditUI.class.getName(), uiContext, null, OprtState.VIEW);
			uiWindow.show();
		}
	}
	private void queryList() throws BOSException{
		...
		filter.getFilterItems().add(new FilterItemInfo("tripApply.id",id));
		...
		TripCostCollection col=TripCostFactory.getRemoteInstance().getTripCostCollection(view);
		for(int i=0;i<col.size();i++){
			IRow row=this.tblTripCost.addRow();
			...
			row.getCell("fromDate").setValue(col.get(i).getFromDate());
			row.getCell("toDate").setValue(col.get(i).getToDate());
			row.getCell("day").setValue(col.get(i).getDay());
			row.getCell("place").setValue(col.get(i).getPlace());
			row.getCell("purpose").setValue(col.get(i).getPurpose());
			row.getCell("transport").setValue(col.get(i).getTransport());
	...
	public void actionAttachment_actionPerformed(ActionEvent e)throws Exception {
		...
		if(getBillStatePropertyName() != null){
			int rowIdx = this.tblTripCost.getSelectManager().getActiveRowIndex();
			ICell cell = this.tblTripCost.getCell(rowIdx, getBillStatePropertyName());
	...
	protected void tblMain_tableClicked(KDTMouseEvent e) throws Exception {
		if (e.getType() == KDTStyleConstants.BODY_ROW && e.getButton() == MouseEvent.BUTTON1 && e.getClickCount() == 2){
			...
			uiContext.put(UIContext.OWNER, this);
			uiContext.put("ID", id);
			IUIWindow uiWindow = UIFactory.createUIFactory(UIFactoryName.NEWTAB).create(TripApplyEditUI.class.getName(), uiContext, null, OprtState.VIEW);
			uiWindow.show();
		}
	}
	@Override
