...
public class ExpenseCostControllerBean extends AbstractExpenseCostControllerBean
{
    private static Logger logger =
        Logger.getLogger("com.kingdee.eas.fdc.contract.app.ExpenseCostControllerBean");
...
protected void _audit(Context ctx, BOSUuid billId) throws BOSException,
	...
	sic.add("supplier.*");
	sic.add("bankNum.name");
	ExpenseCostInfo tripInfo=this.getExpenseCostInfo(ctx, new ObjectUuidPK(billId),sic);
...
//	payment.setPayeeName(tripInfo.getPerson().getName());
	payment.setUsage(tripInfo.getSupplier().getName()+" 费用报销，"+tripInfo.getExpenseType().getAlias()+"，金额"+tripInfo.getAmount()+"元；");
	
	EntityViewInfo evi = new EntityViewInfo();
	filter = new FilterInfo();
    StringBuffer sb = new StringBuffer();
    sb.append("select FAsstActTypeID from T_BD_AsstActTypeDefault where FIsAccountCussent=1 ");
    sb.append(" and FCompanyID='" + payment.getCompany().getId().toString() + "'");
	...
	payment.setPayeeID(tripInfo.getSupplier().getId().toString());
	payment.setPayeeNumber(tripInfo.getSupplier().getNumber());
	payment.setPayeeName(tripInfo.getSupplier().getName());
	payment.setPayeeBank(tripInfo.getReceiveBank());
	
	payment.setPayeeAccountBank(tripInfo.getAccountNumber());
	for(int k=0;k<asstActTypeCollection.size();k++){
		if ("provider".equalsIgnoreCase(asstActTypeCollection.get(k).getAsstHGAttribute())){
			payment.setPayeeType(asstActTypeCollection.get(k));
		}
	}
	...
	payBillEntryInfo.setRemark(tripInfo.getSupplier().getName()+" 费用报销，"+tripInfo.getExpenseType().getAlias()+"，金额"+tripInfo.getAmount()+"元；");
...
private String getNumber(Context ctx,PaymentBillInfo  receivingBillInfo,ExpenseCostInfo revGatherInfo) throws BOSException, EASBizException
{		
